...
from hackyeah_2024_ad_deviniti.application.ai_processor.a_cel_deklaracji_extractor import (
    CelDeklaracjiExtractor,
)
from hackyeah_2024_ad_deviniti.application.ai_processor.a_date_extractor import (
    DateExtractor,
)
from hackyeah_2024_ad_deviniti.application.ai_processor.a_podmiot_extractor import (
    PodmiotExtractor,
)
        ...
        if result.cel_deklaracji is None:
            return TurnResult(
                full_response=TurnResponseFullDto(
                    ...
                    response=TextResponses(
                        agent_1=(
                            "Masz dwie opcje do wyboru. Jeśli składasz deklarację po raz pierwszy, "
            ...
            return TurnResult(
                full_response=TurnResponseFullDto(
                    response_id=str(uuid.uuid1()),
                    response=TextResponses(agent_1=PODMIOT_QUESTION),
                    sources=[],
                    extras=[],
                ),
    ...
    async def process_step(
        ...
        if result.podmiot is None:
            ...
            return TurnResult(
                full_response=TurnResponseFullDto(
                    response_id=str(uuid.uuid1()),
                    response=TextResponses(agent_1=RODZAJ_PODATNIKA_QUESTION),
                    sources=[],
                    extras=[],
                ),
