...
from hackyeah_2024_ad_deviniti.application.ai_processor.b_address_extraction import AdresExtractor
from hackyeah_2024_ad_deviniti.application.ai_processor.b_address_verification_extraction import AdresVerifyExtractor
...
from hackyeah_2024_ad_deviniti.application.ai_processor.b_urzad_skarbowy_extraction_extraction import UsExtractor
...
from hackyeah_2024_ad_deviniti.application.intents import RODZAJ_PODATNIKA, \
    NIP, IDENTYFIKATOR_PODATKOWY, IDENTYFIKATOR_PODATKOWY_WARTOSC, ADDRESS, VERIFY_ADDRESS, URZAD_SKARBOWY_VERIFY, \
...
class IdentyfikatorPodatkowyWartoscStep(DialogStep):
    ...
    async def process_step(self, user_action: UserAction,
                           previous_turns: List[ConversationTurn]) -> TurnResult:
        identyfikator_podatkowy_typ = previous_turns[-1].pcc_3_form.identyfikator_podatkowy
        result = await IdentyfikatorPodatkowyWartoscExtractor().call(user_action.value, identyfikator_podatkowy_typ)
        if result.identyfikator_podatkowy_wartosc is None:
            ...
            form = previous_turns[-1].pcc_3_form
            form.identyfikator_podatkowy_wartosc.value = result.identyfikator_podatkowy_wartosc
            return TurnResult(
                full_response=TurnResponseFullDto(
                    response=TextResponses(
                        agent_1=f"Dziękuję, proszę podaj teraz Adres (miejscowośc, ulicę oraz nummer domu/mieszkania)"),
...
class AddressStep(DialogStep):
    ...
    def build_result_message_for_verify(self, pcc_3_form: Pcc3Form) -> str:
        ...
        merged = ', '.join(results)
        return (f'Podczas weryfikowania formularze znalazłem następujące dane: {merged} – jeśli jakakolwiek informacja '
    ...
    async def process_step(self, user_action: UserAction,
        ...
        if result.not_address:
            return TurnResult(
                full_response=TurnResponseFullDto(
                    response=TextResponses(
                        agent_1=f"Proszę podać adres jeszcze raz"
                    ),
                ),
                intent=ADDRESS,
                pcc_3_form=previous_turns[-1].pcc_3_form
...
class AddressVerifyStep(DialogStep):
    ...
    async def process_step(self, user_action: UserAction,
        ...
        if result.kraj == 'Polska':
            ...
            message = ('Na podstawie adresu wyszukałem Twój Urząd Skarbowy, proszę zweryfikuj czy poprawny. '
